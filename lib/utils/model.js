import { isObject, camelCase, isEmpty, isString } from 'lodash'
...
export default class Model {
  ...
  get (url, { fields, queryParams, multi, headers } = {}) {
  ...
  async handleRequest ({ method, url, body = {}, fields = [], multi = false, queryParams = {}, headers = {} }) {
    ...
    if (!multi) {
      // The case we have no mapping
      if (fields.length === 0) return response.data

      return this.extractModel(fields, response.data)
    }

    response.data = response.data.map(this.extractModel.bind(this, fields))

    return (multi) ? response.data : response
  }
  ...
  extractModel (fields, model, parentModel) {
    ...
    fields.forEach((field) => {
      ...
      const normalizedKey = camelCase(isObject(field) ? field.key : field)
      ...
      const getResult = () => {
        ...
        if (field.mapping) return field.mapping({ model, key: field.key, newModel, parentModel, store: this.$store, localePath: this.localePath, i18n: this.$t })
        else if (field.fields) {
          if (!model[field.key] && field.default) return field.default
          if (Array.isArray(model[field.key]))
            return model[field.key].map(this.extractModel.bind(this, field.fields))
          else
