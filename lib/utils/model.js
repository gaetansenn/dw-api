...
export default class Model {
  ...
  post (url, body, { fields, queryParams, multi, headers } = {}) {
    return this.handleRequest({
      method: 'post',
      body,
      url,
  ...
  delete (url, { fields, queryParams, multi, headers } = {}) {
    return this.handleRequest({
      method: 'delete',
      url,
      fields,
  ...
  patch (url, body, { fields, queryParams, multi, headers } = {}) {
    return this.handleRequest({
      method: 'patch',
      body,
      url,
  ...
  handleRequest ({ method, url, body = {}, fields = [], multi = false, queryParams = {}, headers = {}, catchError = true }) {
    return this.$axios({
      method,
      url,
      headers,
  ...
  extractModel (fields, model, parentModel) {
    ...
    fields.forEach((field) => {
      ...
      const getResult = () => {
        ...
        if (field.mapping) return field.mapping({ model, key: field.key, newModel, parentModel, store: this.$store, localePath: this.localePath, i18n: this.$t, ctx: this.ctx })
        else if (field.fields) {
          if (!model[field.key] && field.default) return field.default
          if (Array.isArray(model[field.key]))
            return model[field.key].map(m => this.extractModel.bind(this, field.fields, m, model)())
          else
